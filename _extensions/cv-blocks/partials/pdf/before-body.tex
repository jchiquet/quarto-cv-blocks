...
% A document that fits on one page (format.cv-blocks-pdf.compact: true)
% needs no running header/footer -- switch off fancyhdr's pagestyle for
% it. Independent of `details`: that one selects long_only/short_only
...
% details: false), so it must keep the fancy pagestyle from

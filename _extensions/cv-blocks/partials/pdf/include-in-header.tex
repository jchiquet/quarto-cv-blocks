...
% Language: \ifenglish drives babel and is available to consuming
% documents for their own EN/FR content. `include-in-header` files are
% spliced in as raw text, not run through pandoc's template engine, so
% $if(...)$ metadata conditionals don't work here. `english` is passed as
% a classoption instead and read back via the LaTeX kernel's
% \@ifclasswith (reports any option passed to \documentclass, whether or
% not the class itself declares it).
\makeatletter
\newif\ifenglish
\@ifclasswith{article}{english}{\englishtrue}{\englishfalse}
\makeatother
